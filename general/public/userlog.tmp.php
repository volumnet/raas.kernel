    ...
    <?php if ($Table->header) { ?>
        <thead>
          ...
          <tr>
            <th>
              <select name="uid[]" id="uid" multiple>
                <?php foreach ($users as $user) { ?>
                    <option value="<?php echo (int)$user->id?>"<?php echo in_array($user->id, (array)$VIEW->nav['uid']) ? ' selected="selected"' : ''?>>
            ...
            <th>
              <select name="filter_method" id="filter_method" multiple>
                <?php foreach ($methods as $method) { ?>
                    <option value="<?php echo htmlspecialchars($method)?>"<?php echo in_array($method, (array)$DATA['filter_method']) ? ' selected="selected"' : ''?>>
            ...
            <th>
              <select name="filter_package[]" id="filter_package" multiple>
                <?php foreach ($packages as $package) { ?>
                    <option value="<?php echo htmlspecialchars($package)?>"<?php echo in_array($package, (array)$DATA['filter_package']) ? ' selected="selected"' : ''?>>
            ...
            <th>
              <select name="filter_module[]" id="filter_module" multiple>
                <?php foreach ($modules as $module) { ?>
                    <option value="<?php echo htmlspecialchars($module)?>"<?php echo in_array($module, (array)$DATA['filter_module']) ? ' selected="selected"' : ''?>>
            ...
            <th>
              <select name="filter_sub[]" id="filter_sub" multiple>
                <?php foreach ($subs as $sub) { ?>
                    <option value="<?php echo htmlspecialchars($sub)?>"<?php echo in_array($sub, (array)$DATA['filter_sub']) ? ' selected="selected"' : ''?>>
            ...
            <th>
              <input type="text" name="action_name" autocomplete="off" class="span2" value="<?php echo $DATA['action_name']?>" />
            </th>
            <th>
              <input type="text" name="element_id" class="span1" value="<?php echo $DATA['element_id']?>" />
