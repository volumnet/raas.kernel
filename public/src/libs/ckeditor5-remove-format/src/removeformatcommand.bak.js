...
export default class RemoveFormatCommand extends Command {
    ...
    *_getFormattingAttributes(item, schema) {
    ...
    /**
     * Проверяет, допустим ли HTML-атрибут у данного элемента
     * @param {element} item Элемент
     * @param {Object|string} attributes Название или объект атрибутов
     */
    isAllowedHtmlAttr(item, attr) {
        if (typeof attr == 'object') {
            for (let key in attr) {
                if (!this.isAllowedHtmlAttr(item, key)) {
                    return false;
                }
            }
            return true;
        ...
        if (['styles', 'classes'].indexOf(attr) != -1) {
            return false;
        }
        return true;
    }
