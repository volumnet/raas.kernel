...
<template>
  <div class="body__background-holder">
    <div class="body__header-outer" v-if="hasActivePackage">
      <header
        class="body__header body__row body__row_header"
        ref="header"
        :class="{
          body__header_fixed: fixedHeader,
          body__header_active: fixedHeaderActive,
        }"
      >
        <menu-packages
          class="body__menu-packages"
          :menu="packagesMenu"
        ></menu-packages>
        <menu-main class="body__menu-main" :menu="mainMenu"></menu-main>
        <menu-user
          class="body__menu-user"
          :user="user"
          :available-languages="availableLanguages"
          v-if="user"
        ></menu-user>
        <menu-mobile
          class="body__menu-mobile"
          :main-menu="mainMenu"
          :left-menu="leftMenu"
          :user="user"
          :available-languages="availableLanguages"
          :packages-menu="packagesMenu"
        ></menu-mobile>
      </header>
    </div>
    <div class="body__row body__row_main">
      <main class="body__main">
        <aside class="body__left" ref="leftPane">
          <menu-left
            class="body__menu-left"
            ref="leftPaneFloat"
            v-if="leftMenu"
            :menu="leftMenu"
            :style="{ marginTop: floatingMenuMargin + 'px' }"
          ></menu-left>
        </aside>
        <article
          class="body__content"
          :class="{ body__content_sided: !!leftMenu }"
        >
          <raas-breadcrumbs
            class="body__breadcrumbs"
          ...
          <div class="body__content-header">
            ...
            <menu-management
              class="body__menu-management"
              v-if="managementMenu && managementMenu.length"
              :menu="managementMenu"
            ></menu-management>
          </div>
        ...
        <footer class="body__footer">
          ...
          <div class="body__copyrights">
            ...
            <a href="https://www.volumnet.ru/" target="_blank">
              {{ translations.VOLUME_NETWORKS }} </a
    ...
    <raas-filemanager ref="filemanager" />
  </div>
</template>
...
export default {
  ...
  mounted() {
    this.leftPaneTop =
      $(this.$refs.leftPane).offset() && $(this.$refs.leftPane).offset().top; // Здесь, потому что нужно задать до отображения для формирования CSS-классов
  ...
  computed: {
    ...
    floatingMenuHeight() {
      return (
        ...
        (this.$refs.leftPaneFloat
          ? $(this.$refs.leftPaneFloat.$el).outerHeight()
          : 0)
      ); // scrollTop для привязки к скроллингу
    },
    /**
    ...
    floatingMenuMarginByBottom() {
      let margin =
        this.floatingMenuBottomEdge -
        this.leftPaneTop -
        this.floatingMenuHeight;
      margin = Math.max(0, margin);
      margin = Math.min(margin, this.maxFloatingMenuMargin);
      return margin;
    },
